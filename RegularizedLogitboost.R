...
library(pROC)
library(caTools)
library(parallelly)  # For more robust parallel processing
library(foreach)

# [LogitBoostTesting function remains the same as in original code]
...
# [predict.LogitBoostTesting function remains the same]
...
# [LogitBoostRegularizedModel caret definition remains the same]
...
# [customSummary function remains the same]
...
# ---------------------------------------------------
# UPDATED MAIN ANALYSIS CODE WITH ADAPTIVE LR PARAMETERS
# ---------------------------------------------------

# Assume data.dfPH, biomarkers, confounders exist
...
# iterations_to_test  <- c(10, 12,13)
# lambda_values       <- c(0.001, 0.01)
# nu_values           <- c(0.01, 0.05)  # Base learning rates
# gamma_values        <- c(0.01, 0.05)  # Inverse-decay rates
# adaptLR_values      <- c(TRUE, FALSE)            # Test both adaptive and fixed
# subsample_values    <- c(0.3, 0.5)
# mtry_values         <- c(0.3, 0.5)



# UPDATED: Result columns to include adaptive LR parameters
base_cols <- c(
...
# UPDATED: Calculate total iterations including new parameters
...
# Data quality checks (same as before)
...
# Helper function for feature importance (same as before)
...
# UPDATED: Grid search with adaptive learning rate parameters
  ...
  foreach(iter_val       = iterations_to_test,.combine = 'rbind',
          .packages      = c("caret","pROC","dplyr","glue")) %dopar% {
            ...
            tryCatch({
              ...
              result_row$gamma     <- gamma_val    # NEW
              result_row$adaptLR   <- adaptLR_val  # NEW
              result_row$subsample <- sub_val
              result_row$mtry      <- mtry_val
              
              # UPDATED: Tuning parameters now include gamma and adaptLR
...
if (!is.null(all_results) && nrow(all_results) > 0) {
  ...
  # UPDATED: Best models analysis including adaptive LR effects
  ...
  # Parameter sensitivity analysis for adaptive LR
  if (nrow(adaptive_results) > 0) {
    ...
    # Best nu values analysis for adaptive models
  ...
  # UPDATED: Save with adaptive LR in filename
  ...
  # UPDATED: Print adaptive vs fixed learning rate comparison
...
# ADDITIONAL: Function to analyze learning rate decay patterns
...
# Example: Visualize learning rate decay for different gamma values
